...
*********************************************************/
/*W5500SPI相关函数*/
void IINCHIP_WRITE(struct device *dev, uint32 addrbsb,  uint8 data);		    /*写入一个8位数据到W5500*/
uint8 IINCHIP_READ(struct device *dev,uint32 addrbsb);						    /*从W5500读出一个8位数据*/
uint16 wiz_write_buf(struct device *dev,uint32 addrbsb,uint8* buf,uint16 len);	/*向W5500写入len字节数据*/
uint16 wiz_read_buf(struct device *dev,uint32 addrbsb, uint8* buf,uint16 len);	/*从W5500读出len字节数据*/

void iinchip_init(struct device *dev);                                     // reset iinchip
void socket_buf_init(struct device *dev,uint8 * tx_size, uint8 * rx_size); // setting tx/rx buf size
uint8 getISR(struct device *dev,uint8 s);
void putISR(struct device *dev,uint8 s, uint8 val);
uint16 getIINCHIP_RxMAX(uint8 s);
uint16 getIINCHIP_TxMAX(uint8 s);
void setMR(struct device *dev,uint8 val);
void setRTR(struct device *dev,uint16 timeout); 			               // set retry duration for data transmission, connection, closing ...
void setRCR(struct device *dev,uint8 retry);                               // set retry count (above the value, assert timeout interrupt)
void clearIR(struct device *dev,uint8 mask);                               // clear interrupt
uint8 getIR( struct device *dev);
void setSn_MSS(struct device *dev,SOCKET s, uint16 Sn_MSSR);               // set maximum segment size
uint8 getSn_IR(struct device *dev,SOCKET s);                               // get socket interrupt status
uint8 getSn_SR(struct device *dev,SOCKET s);                               // get socket status
uint16 getSn_TX_FSR(struct device *dev,SOCKET s);                          // get socket TX free buf size
uint16 getSn_RX_RSR(struct device *dev,SOCKET s);                          // get socket RX recv buf size
void setSn_TTL(struct device *dev,SOCKET s, uint8 ttl);
void send_data_processing(struct device *dev,SOCKET s, uint8 *wizdata, uint16 len);
void recv_data_processing(struct device *dev,SOCKET s, uint8 *wizdata, uint16 len);

void setGAR(struct device *dev,uint8 * addr);                              // set gateway address
void setSUBR(struct device *dev,uint8 * addr);                             // set subnet mask address
void setSHAR(struct device *dev,uint8 * addr);                             // set local MAC address
void setSIPR(struct device *dev,uint8 * addr);                             // set local IP address
