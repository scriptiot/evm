...
#undef LOG_E
#undef LOG_D
#define LOG_E http_error
#define LOG_D http_debug
...
static int webclient_send(struct webclient_session* session, const void *buffer, size_t len, int flag)
{
    ...
    int ret = 0;
    if (session->is_https) {
        ret = ssl_client_write(&session->ssl->ssl, (const unsigned char *)buffer, len);
    }else{
        ret = send(session->socket, buffer, len, flag);
    }
...
static int webclient_recv(struct webclient_session* session, void *buffer, size_t len, int flag)
{
    ...
    int ret = 0;
    if (session->is_https) {
        ret = ssl_client_read(&session->ssl->ssl, (const unsigned char *)buffer, len);
    }else{
        ret = recv(session->socket, buffer, len, flag);
    }
...
static int webclient_read_line(struct webclient_session *session, char *buffer, int size)
{
    ...
    if (count > size)
    {
        ...
        return -WEBCLIENT_READ_LINE_ERROR;
    }
                                     ...
                                     const char *url, const char **request)
{
    ...
    {
        rc = -WEBCLIENT_RESOLVE_ADDRESS_NOT_HTTP;
        goto __exit;
    }

    char* host_path_right = strchr(host_addr, 0x3F);
    char host_path[255] = {0};
    if (host_path_right != NULL){
        memcpy(host_path, host_addr, strlen(host_addr) - strlen(host_path_right));
        host_addr = host_path;
    }
    ...
    if (host_addr[0] == '[')
    {
        ...
        if (!ptr)
        {
            rc = -WEBCLIENT_RESOLVE_ADDRESS_IPV6_ERROR;
            goto __exit;
        }
    ...
    if (host_path_right != NULL){
        if (isHttp){
            *request = path_ptr ? strstr(url+7, "/") : "/";
        }
        if (isHttps){
            *request = path_ptr ? strstr(url+8, "/") : "/";
        }
    }else{
    ...
    {
        rc = -WEBCLIENT_RESOLVE_ADDRESS_LENGTH_ERROR;
        goto __exit;
    }
    ...
    /* get host address ok. */
    {
        ...
        if (!host_addr_new)
        {
            rc = -WEBCLIENT_RESOLVE_ADDRESS_MALLOC_ERROR;
            goto __exit;
        }
    ...
    /* resolve the host name. */
    {
        ...
        if (ret != 0)
        {
            ...
            rc = -WEBCLIENT_RESOLVE_ADDRESS_GETADDRINFO_ERROR;
            goto __exit;
        }
    }
...
//        goto __exit;
    ...
    {
        rc = -WEBCLIENT_CONNECT_ADDRESS_ERROR;
        goto __exit;
    }
    ...
    else
    {
        ...
        rc = -WEBCLIENT_CONNECT_FAILED_ERROR;
        goto __exit;
    }
...

    {
        ...
        if (!session->ssl && session->is_https) {
            http_debug("Perform ssl_client_init...");
            session->ssl = ssl_client_init(session->socket);

            if (session->ssl == NULL) {
                http_error("Try build new SSL client failed");
                return -WEBCLIENT_CONNECT_SSL_FAILED;
            }
        }
    }
...
int webclient_header_fields_add(struct webclient_session *session, const char *fmt, ...)
{
    ...
    if (length < 0)
    {
        ...
        return -WEBCLIENT_FIELDS_ADD_ERROR;
    }
    va_end(args);
...
int webclient_get_position(struct webclient_session *session, const char *URI, int position)
{
    ...
    {
        rc = -WEBCLIENT_GET_POSITION_ERROR;
        return rc;
    }
...
int webclient_post(struct webclient_session *session, const char *URI, const void *post_data, size_t data_len)
{
    ...
    if ((post_data != RT_NULL) && (data_len == 0))
    {
        ...
        return -WEBCLIENT_POST_DATA_LENGTH_ERROR;
    }
...
static int webclient_clean(struct webclient_session *session)
{
    if (session->ssl) {
        ...
        ssl_client_shutdown(session->ssl);
    }
#ifdef WEBCLIENT_USING_MBED_TLS
    if (session->tls_session)
    {
...
int webclient_request_header_add(char **request_header, const char *fmt, ...)
{
    ...
    {
        header = web_calloc(1, WEBCLIENT_HEADER_BUFSZ);
        if (header == RT_NULL)
        {
    ...
    if (length < 0)
    {
        ...
        return -WEBCLIENT_REQUEST_HEADER_ADD_ERROR;
    }
    va_end(args);
    ...
    if (rt_strlen(header) >= WEBCLIENT_HEADER_BUFSZ)
    {
        ...
        return -WEBCLIENT_REQUEST_HEADER_ADD_BUFFER_ERROR;
    }
...
int webclient_request(const char *URI, const char *header, const void *post_data, size_t data_len, void **response, size_t *resp_len)
{
    ...
    if (post_data == RT_NULL && response == RT_NULL)
    {
        ...
        return -WEBCLIENT_REQUEST_ERROR_1;
    }

    if ((post_data != RT_NULL) && (data_len == 0))
    {
        LOG_E("input post data length failed");
        return -WEBCLIENT_REQUEST_ERROR_2;
    }
        ...
        (response == RT_NULL && resp_len != RT_NULL))
    {
        ...
        return -WEBCLIENT_REQUEST_ERROR_3;
    }
